...
DEFAULT_FILETYPES = ('.py', '.txt', '.sh', '.js', '.pl', '.c', '.h', '.cpp')
...
usage_str = """{filename} v. {version}
    ...
    Usage:
        {filename} -h
        {filename} <searchterm> [<target>...] [-a | -t <filetypes>] [-d] [-n] [-p] [-r]

    Options:
        <searchterm>                        : Regex to match in file or files.
        <target>                            : Target file, or directory to search in. (*.py will work also.)
                                              Default: Current directory
        -a,--all                            : Search all file types. (Same as '--types all')
                                              Can be slow, and searching binary files doesn't work well.
        -d,--debug                          : Print more debug info.                                      
        -h,--help                           : Show this message.
        -n,--nocolors                       : Don't use colors at all.
        -p,--print                          : Print ALL file names before searching (even non-matching, it's slow!).
        -t <filetypes>,--types <filetypes>  : Only search files with these extensions. (Comma-separated list)
                                              Default: {filetypes}
        -r,--reverse                        : Only show files that don't match.
    
    Notes:
        Shell-expansion, --types, and recursing sub-directories:
            By default, if <target> is a directory, or contains a directory, any sub-directories of
            the target will be searched. If no target is specified, the current directory is the target.
            You can override this behaviour by using expansion to search only certain files
            in the current directory.
            Examples:
                {filename} 'foo' == {filename} 'foo' *
                ...since directories are expanded with *, both recurse into sub-directories.
                
                {filename} 'foo' * --types .py != {filename} 'foo' *.py
                ...all sub-directories will be included in * but only .py files are searched, 
                   where *.py usually excludes sub-directories.
                   
                {filename} 'foo' *.py --types .txt == Nothing.
                ...all .py files will be sent to {filename}, but only .txt will be searched.
                
            If {filename} sees a directory in the <target>, it walks all sub-directories of it.
            
        Stackless:
            This script is setup to run with stackless-python (/usr/bin/stackless), but you can use
            any python executable that will successfully do 'import stackless'.
        
""".format(filename=SCRIPTFILE, version=__VERSION__, filetypes=', '.join(DEFAULT_FILETYPES))
...
class MatchLine():
    ...
    def __str__(self):
        # trim tabs/spaces characters, replaces with '...' (for str() and repr() only)
        if self.text.startswith(' ') or self.text.startswith('\t'):
            stext = '... ' + self.text.strip(' ').strip('\t')
        else:
            stext = self.text
...
class MatchCollection():
    """ collection of MatchInfo() (self.items is list of MatchInfo() with helper functions) """
    ...
    def __add__(self, other):
        ...
        if isinstance(other, MatchCollection):
            ...
            errmsg = '{} cannot be added to a MatchCollection, only other MatchCollections.'.format(str(type(other)))
    ...
    def total_lines(self):
        ...
        if self.items:
            for matchinf in self.items:
                if matchinf.lines: total += len(matchinf.lines)
        return total
...
class ColorCodes(object):
    """ This class colorizes text for an ansi terminal. Inspired by Colorama (though very different) """
    
    def __init__(self):
        self.codes = {'fore': {'black': '30', 'red': '31', 'green': '32', 'yellow': '33',
                               'blue': '34', 'magenta': '35', 'cyan': '36', 'white': '37', 
                               'reset': '39'},
                      'back': {'black': '40', 'red': '41', 'green': '42', 'yellow': '43', 'blue': '44',
                               'magenta': '45', 'cyan': '46', 'white': '47', 
                               'reset': '49'},
                      'style': {'bright': '1', 'dim': '2', 'normal': '22', 'none': '22',
                                'reset_all': '0',
                                'reset': '0'},
                      }
    ...
    def colorize(self, text, style=None, back=None, fore=None):
        ...
        """
        return '{codes}{txt}'.format(codes=self.color_code(style=style, back=back, fore=fore),
                                     txt=text)
        
    def colorizepart(self, text, style=None, back=None, fore=None):
        """ Same as colorize, but adds a style->reset_all after it. """
        return '{colrtxt}{reset}'.format(colrtxt=self.colorize(text, style=style, back=back, fore=fore),
                                         reset=self.color_code(style='reset_all'))


def searcher_run(searchpat, kw_args, matches=None, colorizer=None, channel_=None):
    ...
    # searches the file, and reports matches. also, updates the MatchesCollection() (matches).
    stay = True    
    while stay:
        ...
        if filematch:
            ...
            if colorizer:
                filepath = colorizer.colorizepart(filename, style='bright', fore='blue')
            else:
                filepath = filename
                
            print('\n{}:'.format(filepath))
            # Lines are highlighted in get_file_matches() if colorizer was passed.
            for matchline in filematch.iterlines():
                print('    {}'.format(matchline))

       
def filler_run(startpath, filetypes, channel_=None):
   """ task to start finding files, sends to searcher_run tasklet via channel """
...
def get_multi_matches(target, searchpat, **kwargs):
    """ Searches multiple files/dirs passed in with * expansion or just repeating args 
        Arguments:
            target        : List of files/dirs to search.
            searchpat     : Pre-compiled regex pattern to search with.
        Keyword Arguments:
            reverse       : If True, include lines that DONT match. (Default: False)
            print_status  : If True, print file name before searching. (Default: False)
            filetypes     : List of file extensions to search. (Default: None (all files))
            debug         : Print more info (UnicodeDecodeError right now)
            colorizer     : ColorCodes() class to highlight with, or None for no highlighting.
    """
    ...
    targettask = stackless.tasklet(targets_run)(target, filetypes, channel)
    searchertask = stackless.tasklet(searcher_run)(searchpat, kwargs, matches=matches, colorizer=colorizer, channel_=channel)
    try:
        stackless.run()
    except KeyboardInterrupt as excancel:
        if colorizer:
...
def get_dir_matches(startpath, searchpat, **kwargs):
    ...
    fillertask = stackless.tasklet(filler_run)(startpath, filetypes, channel)
    searchertask = stackless.tasklet(searcher_run)(searchpat, kwargs, matches=matches, colorizer=colorizer, channel_=channel)
    try:
        stackless.run()
    except KeyboardInterrupt as excancel:
...
def get_file_matches(filename, searchpat, **kwargs):
    """ searches a single file for a match. returns single MatchInfo() 
        Arguments:
            ...
            searchpat     :  Pre-compiled regex pattern to search with.
        Keyword Arguments:
            reverse       :  If True, return lines that DONT match. (Default: False)
            max_len       :  Exclude lines longer than max_len (Default: 0 (no max))
            print_status  :  If True, print file name before searching. (Default: False)
            debug         :  Print more info (UnicodeDecodeError right now)
            colorizer     :  ColorCodes() class to highlight with, or None for no highlighting.
    """
    ...
    # predetermine functions to use, moves several if statements OUTSIDE of the loop.
    #
    # Max Length used?
    # fadd is a FileAddData() with (fileinf, line, lineno)
    with_maxlen = lambda fadd: fadd.info.addif(len(fadd.line) < max_len, fadd.line, fadd.lineno)
    ...
    for lineindex in range(len(filelines)):
        ...
        if match_func(re_match):
            ...
            if usecolors:
                text = re_match.group()
                if text:
                    line = replace_(line, text, colorizer.colorizepart(text, **highlight_args))
                
            # use pre-determined add method (send data as tuple to get around lambda if restriction)
            add_func(FileAddData(info=file_info, line=line, lineno=lineindex + 1))
...
def fill_queue(startpath = None, filetypes = None):
    ...
    if filetypes:
        ...
        # Walk the directory,yielding ALL files (not good to search binary files)
        for root, dirs, files in os.walk(startpath):
...
def is_none(obj):
    """ Helper for get_file_matches, with reverse flag -> match_func = is_none(match) """
    return (obj is None)

def is_not_none(obj):
    """ Helper for get_file_matches, with no reverse flag -> match_func = is_not_none(match) """
...
def parse_filetypes_arg(originalargd):
    ...
    if originalargd['--all']:
        ...
        if originalargd['--types']:
            filetypes = originalargd['--types'].replace('"', '').replace("'", '').replace(' ', '')
...
def print_matches(matches, printlines=True, colorizer=None):
    """ print results, from list of single MatchInfo, or MatchCollection list.
        Arguments:
            matches     : A MatchCollection() (dir search) or list of 1 MatchInfo() object (single-file search).
            
        Keyword Arguments:
            printlines  :  If True, print every line that matched. (Default: True)
            colorizer   :  A ColorCodes() class to highlight with, or None for no highlighting.
    """
    ...
    # Helper functions (used more than once, in different situations, helps to stay uniform.)
    def no_matches(filecount=None):
        """ Helper function, handles no matches found. """
        nomatchstr = '\nNo matches found.'
        if colorizer:
            nomatchstr = colorizer.colorizepart(nomatchstr, style='bright', fore='red')
        print(nomatchstr)
    ...
    def print_searchedcount(searchcount=None):
        ...
        if colorizer:
            filecount = colorizer.colorizepart(searchcount, **colorizer_args)
        print('Searched {} {}.'.format(searchcount, searchedplural))
        
    # Check matches object (no matches object at all/no matches with files searched).
    if matches is None:
        no_matches()
        return 0
    ...
    if colorizer:
        filecount = colorizer.colorizepart(filecount, **colorizer_args)
    
    # Dir or Single File matches?
    if hasattr(matches, 'total_lines'):
        # match collection (Actual MatchCollection() with helper attributes/functions)
        linecount = str(matches.total_lines())
        if colorizer:
            linecount = colorizer.colorizepart(linecount, **colorizer_args)
        ...
        if printlines:
            ...
            if colorizer:
                singlefilename = colorizer.colorizepart(singlefilename, style='bright', fore='blue')
...
def main(argd):
    ...
    try:
        ...
        print_fail('Invalid search term!: {}'.format(searchterm), exc=ex, retcode=1)
    
    # Colorize search type info, or use normal strings
    if len(target) > 1:
        # Multiple targets, may be files or directories (could be shell-expanded *.whatever)
        targetlenstr = colors.colorizepart(str(len(target)), style='bright', fore='blue') if colors else str(len(target))
        targettypestr = colors.colorizepart('files/directories', fore='blue') if colors else 'files/directories'
        targetstr = '{} {}'.format(targetlenstr, targettypestr)
        if colors:
            targetstr = colors.colorizepart(targetstr, fore='blue')
    else:
        targetstr = colors.colorizepart(target[0], fore='blue') if colors else target[0]
        
    searchtermstr = colors.colorizepart(searchterm, fore='blue') if colors else searchterm
    reversestr = '    *Reverse searching, looking for lines that don\'t match.'
    reversestr = colors.colorizepart(reversestr, fore='red') if colors else reversestr
    ...
    if len(target) == 1 and os.path.isfile(target[0]):
        ...
        if argd['--reverse']:
            print(reversestr)
        matches = [get_file_matches(target, searchpat, reverse=argd['--reverse'],
                                                      debug=argd['--debug'],
                                                      colorizer=colors)]
        ...
        if colors:
            if filetypes:
                filetypestr = ', '.join([colors.colorizepart(stype, fore='blue') for stype in sorted(filetypes)])
            else:
                filetypestr = colors.colorizepart('All Files', fore='blue')
        else:
        ...
        # Do the search
        matches = get_dir_matches(target[0], searchpat, reverse=argd['--reverse'], 
                                                     filetypes=filetypes,
                                                     debug=argd['--debug'],
                                                     colorizer=colors)
        ...
        # Do the search
        matches = get_multi_matches(target, searchpat, reverse=argd['--reverse'],
                                                       filetypes=filetypes,
                                                       debug=argd['--debug'],
                                                       colorizer=colors)
        # Matches are printed as they are found, per file
    ...
    runtime = str(runtime)
    if colors:
        formattedtime = '({} seconds)'.format(colors.colorizepart(runtime, style='bright', fore='cyan'))
    else:
