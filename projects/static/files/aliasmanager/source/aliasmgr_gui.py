...
class winMain():
    ...
    def mnuNewFile_button_release_event_cb(self, widget, signal_id, data=None):
        ...
        if amutil.create_blank_file(sfile, True):
            settings.setsave("aliasfile", sfile)
            self.set_filename(sfile)
            self.load_aliases(True)
            self.stat_settext(
                "Created new alias file: " + amutil.filename_safe(sfile))
        else:
    ...
    def mnuSelFile_button_release_event_cb(self, widget, signal_id, data=None):
        ...
        if os.path.isfile(sfile):
            settings.setsave("aliasfile", sfile)
            self.stat_settext("Alias file: " + amutil.filename_safe(sfile))
        ...
        if integrator.bashrc is None:
            if not integrator.find_bashrc():
                dlg.msgbox('\n'.join((
                    ...
                    "You may need to create a file called",
                    ".bashrc or bash.bashrc in /home {} or /etc/.",
                    dlg.error)).format(
                        integrator.home))
...
# Local.Functions --------------------------------------------------
    ...
    def save_file(self, sfilename=None):
        ...
        lst_header.append("# -Christopher Welborn\n\n")
        ...
        # because they are single lines. multiline functions can't be sorted
        # like that.
        ...
        try:
            ...
            dlg.msgbox("Unable to copy to destination: " +
                       amutil.filename_safe(sfilename), dlg.error)
